...
impl PythonParser {
    ...
    fn find_lambda_assignments(
        ...
        let mut functions = Vec::new();
        let name_idx = self.queries.lambda_assignments.capture_index_for_name("name").unwrap();
        ...
        while let Some(m) = { matches.advance(); matches.get() } {
            for cap in m.captures {
                ...
                let node = cap.node;
                let assignment_node = node.parent().unwrap();
                let lambda_node = assignment_node.child_by_field_name("right").unwrap();
                let name = get_node_text(&node, source).to_string();
    ...
    fn find_imports(&self, source: &[u8], root: &Node, cursor: &mut QueryCursor) -> Vec<ImportData> {
        ...
        let from_idx = self.queries.imports.capture_index_for_name("from_import_stmt").unwrap();
        ...
        while let Some(m) = { matches.advance(); matches.get() } {
            for cap in m.captures {
                ...
                let call_node = {
                    ...
                    if p.kind() == "call" { p } else { p.parent().unwrap() }
                };
