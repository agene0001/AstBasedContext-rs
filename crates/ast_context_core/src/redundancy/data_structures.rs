...
pub(super) fn detect_vec_used_as_set(
    ...
    for &(idx, node) in &ctx.functions {
        ...
        for line in src.lines() {
            ...
            if let Some(bracket_pos) = trimmed.find('[') {
                if bracket_pos > 0 {
                    ...
                    let var = before.split(|c: char| !c.is_alphanumeric() && c != '_' && c != '.').last().unwrap_or("");
                    if !var.is_empty() {
                        indexed.push(var);
                    }
                }
        ...
        for &recv in &appended {
            if contained.iter().any(|&c| c == recv) && !indexed.iter().any(|&i| i == recv) {
        ...
        for line in src.lines() {
            ...
            for pattern in &insert_patterns {
                if let Some(recv) = extract_receiver(trimmed, pattern) {
                    ...
                    if let Some(after_method) = trimmed.find(pattern).and_then(|p| {
                        Some(&trimmed[p + pattern.len()..])
                    }) {
