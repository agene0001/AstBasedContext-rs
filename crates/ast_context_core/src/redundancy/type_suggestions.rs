...
pub(super) fn suggest_parameter_structs(
    ...
    for &(idx, name, args) in &candidates {
        let mut key: Vec<String> = args.iter().cloned().collect();
        key.sort();
    ...
    for i in 0..candidates.len() {
        ...
        for j in (i + 1)..candidates.len() {
            ...
            if shared.len() >= 4 {
                ...
                let mut key_a: Vec<String> = candidates[i].2.iter().cloned().collect();
                key_a.sort();
                let mut key_b: Vec<String> = candidates[j].2.iter().cloned().collect();
                key_b.sort();
                if key_a == key_b {
                    continue;
                }
