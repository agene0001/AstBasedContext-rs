...
pub struct AnalysisConfig {
    ...
    /// Minimum IDF-weighted structural (AST-shape) cosine for a lexical
    /// near-duplicate / structural-similar match to be confirmed. Higher =
    /// stricter = fewer false positives. Only applied to annotated graphs.
    pub structural_confirm_threshold: f64,
...
impl Default for AnalysisConfig {
    fn default() -> Self {
        Self {
            ...
            structural_confirm_threshold: 0.50,
...
impl FindingKind {
    ...
    /// Compact legend entry of the form `"<code>=<label>"` used in CLI and MCP
    /// output (e.g. `"ND=near-dup"`).
    ///
    /// This is the single source of truth for a finding's display code and label.
    /// `short_code()` derives the bare code from this, so adding a new finding
    /// kind only requires adding one arm here.
    pub fn legend_entry(&self) -> &'static str {
        match self {
    ...
    /// Short tag code (e.g. `"P"`, `"ND"`) for compact finding output.
    ///
    /// Derived from `legend_entry()` so the code and legend can never drift.
    pub fn short_code(&self) -> &'static str {
        match self.legend_entry().split_once('=') {
            Some((code, _)) => code,
            None => self.legend_entry(),
        }
    }
}
