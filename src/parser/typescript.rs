...
impl TypeScriptParser {
    ...
    fn find_functions(&self, source: &[u8], root: &Node, path: &Path, cursor: &mut QueryCursor) -> Vec<FunctionData> {
        ...
        let Some(fn_idx) = self.queries.functions.capture_index_for_name("function_node") else { return functions; };
    ...
    fn find_classes(&self, source: &[u8], root: &Node, path: &Path, cursor: &mut QueryCursor) -> Vec<ClassData> {
        ...
        let Some(class_idx) = self.queries.classes.capture_index_for_name("class_node") else { return classes; };
    ...
    fn find_interfaces(&self, source: &[u8], root: &Node, path: &Path, cursor: &mut QueryCursor) -> Vec<InterfaceData> {
        ...
        let Some(iface_idx) = self.queries.interfaces.capture_index_for_name("interface_node") else { return interfaces; };
